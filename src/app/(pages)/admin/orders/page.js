...
export default function AdminOrdersPage() {
  const [counts, setCounts] = useState({ total: 0, pending: 0, delivered: 0, failed: 0 });
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      setOrdersError(null);
      try {
        const res = await fetch(`/api/admin/orders?status=${encodeURIComponent(currentStatus)}&limit=5`);
  ...
  const loadMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/orders?status=${encodeURIComponent(currentStatus)}&limit=5&after=${nextCursor}`
      );
