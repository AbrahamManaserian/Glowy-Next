...
function OptionalInputs({ categories, inputs, optionalInputs, setOptionalInputs, requiredFields }) {
  return (
    <>
      {Object.keys(categories?.[inputs.category]?.[inputs.subCategory] || []).map((key, index) => {
        if (Array.isArray(categories?.[inputs.category]?.[inputs.subCategory][key]) && key !== 'brands') {
          return (
            <FormControl
              key={index}
              sx={{
              ...
              size="small"
              error={!!requiredFields && !optionalInputs[key]}
            >
              <InputLabel>{key}</InputLabel>
              <Select
                name={key}
                value={optionalInputs[key] || ''}
                label={key}
                onChange={(e) => setOptionalInputs({ ...optionalInputs, [key]: e.target.value })}
              >
                {(categories?.[inputs.category]?.[inputs.subCategory][key]).map((item, index) => {
                  return (
                    <MenuItem key={index} value={item.type ? item.type : item}>
        ...
        >
          {inputs.subCategory &&
            categoriesObj[inputs.category][inputs.subCategory].type.map((item, index) => {
              return (
                <MenuItem sx={{ textTransform: 'capitalize' }} key={index} value={item.type}>
                  {item.type}
                </MenuItem>
              );
            })}
