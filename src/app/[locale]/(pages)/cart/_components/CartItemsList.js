  ...
  return (
    <>
      {Object.keys(cart.items).map((id, index) => {
        if (cart.items[id].options) {
          // console.log(cartDetails[id]);
          return Object.keys(cart.items[id].options).map((optionId, optionIndex) => {
            const quantity = cart.items[id].options[optionId];

            // const price = cartDetails?.[id].availableOptions.find((o) => o[id].optionKey === optionId)?.price;
            const price =
              cartDetails?.[id]?.availableOptions?.find((o) => o[cartDetails[id].optionKey] === optionId)
                ?.price || cartDetails?.[id]?.price;
            // console.log( price);
            return (
              <Box key={id + '-' + optionId}>
                <DetailedCartItem
                  item={{ ...cart.items[id], quantity: quantity, price: price }}
                  id={id}
                  option={optionId}
