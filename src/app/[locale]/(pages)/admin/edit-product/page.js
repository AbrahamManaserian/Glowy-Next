...
import { categoriesObj, initialInputs, initialOptionInputs } from '../add-product/page';
...
export default function EditProductPage() {
  ...
  const [lastProductIdFromDB, setLastProductIdFromDB] = useState(null);
  const [categoryData, setCategoryData] = useState({ brands: [], sizes: [], perfumeNotes: [] });
  const [createOptionsWithId, setCreateOptionsWithId] = useState(true);
  const [editInitialProductOption, setEditInitialProductOption] = useState(null);
  const [editingOptionValues, setEditingOptionValues] = useState(null);
  ...
  const handleRemoveOption = (id) => {
    ...
    if (inputs.availableOptions.length === 1) {
      ...
      setOptions({ ...options, disableOption: false });
    } else {
      setInputs((prev) => ({ ...prev, availableOptions: updatedOptions }));
    }
  ...
  const editProduct = async () => {
    ...
    try {
      ...
      if (inputs.optionHasId) {
        allProductids[product] = initialProduct.brand + ' - ' + initialProduct.model;

        let startId = lastProductIdFromDB;
        let lastProductId = lastProductIdFromDB;
                ...
                await uploadBytes(mainRef, item.mainImage.file).then((snapshot) => {
                  return getDownloadURL(snapshot.ref).then((url) => {
                    mainOptionImage = {
                      ...item.mainImage,
                      file: url,
                      src: `glowyProducts/${item.id}/main`,
                    };
                  });
                });
        ...
        setInputs(() => structuredClone(initialInputs));
        setOptions(() => structuredClone(initialOptionInputs));
        setLastProductIdFromDB(lastProductId);
        setProducts((prev) => ({ ...prev, ...allProductids }));
        setRequiredFields(false);
        setRequiredOption(false);
        setDeletedImages([]);
        setProduct(null);
        ...
        setOptionKeyValue('');
        setRemovedOption([]);
        setCategoryData({ brands: [], sizes: [], perfumeNotes: [] });
        setLoading(false);
      } else {
        const availableOptionItems = options.availableOptions.map((option, index) => {
          return {
            ...
            id: index + initialProduct.availableOptions.length,
          };
        });

        initialProduct.availableOptions.push(...availableOptionItems);

        const productRef = doc(db, 'glowyProducts', initialProduct.category, 'items', product);
        const allProductsRef = doc(db, 'allProducts', product);
        const detailRef = doc(db, 'details', 'projectDetails');
        ...
        const itemIdName = initialProduct.brand + ' - ' + initialProduct.model;

        await updateDoc(detailRef, {
          ['allProductsIds.' + product]: itemIdName,
        });

        const needUpdateSize = initialProduct.availableOptions
          .filter((item) => !categoryData.sizes.includes(item.size) && item.size)
          .map((i) => i.size);

        if (!categoryData.sizes.includes(inputs.size) && inputs.size) {
          needUpdateSize.push(inputs.size);
        }

        if (needUpdateSize[0] || (!categoryData.brands.includes(inputs.brand) && inputs.brand)) {
          const docref = doc(db, 'details', inputs.category);

          await updateDoc(docref, {
            sizes: arrayUnion(...needUpdateSize),
            brands: arrayUnion(inputs.brand),
          });
        }
        setInputs(() => structuredClone(initialInputs));
        setOptions(() => structuredClone(initialOptionInputs));
        setProducts((prev) => ({ ...prev, [product]: itemIdName }));
        setRequiredFields(false);
        setRequiredOption(false);
        setDeletedImages([]);
        setProduct(null);
    ...
    if (product) {
      ...
      getProduct(product).then((data) => {
        if (data.brand) {
          ...
          setCreateOptionsWithId(Boolean(data.optionHasId));
          ...
          if (data.availableOptions.length > 0) {
            ...
            setOptions({ ...structuredClone(initialOptionInputs), disableOption: false });
            setOptionKeyValue('');
          }
        } else {
          setInputs(structuredClone(initialInputs));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setProducts(data?.projectDetails?.allProductsIds || {});
      setLastProductIdFromDB(data?.projectDetails?.lastProductId || null);
  ...
  useEffect(() => {
    if (editedProduct.category) {
      const fetcheCategoryData = async () => {
        try {
          const categoryDocref = doc(db, 'details', editedProduct.category);
          const categoryData = await getDoc(categoryDocref);
          if (categoryData.exists()) {
            setCategoryData({ perfumeNotes: [], brands: [], sizes: [], ...categoryData.data() });
          } else {
            setCategoryData({ perfumeNotes: [], brands: [], sizes: [] });
          }
        } catch (error) {
          console.log(error);
          setCategoryData({ perfumeNotes: [], brands: [], sizes: [] });
    ...
    >
      ...
      <Button
        ...
        onClick={async () => {
          ...
          console.log('editInitialProductOption', editInitialProductOption);
      ...
      <Autocomplete
        size="small"
        options={Object.keys(products).sort() || []}
        getOptionLabel={(option) => `${option} - ${products[option]}`}
        value={product}
      ...
      >
          ...
          {inputs.availableOptions?.length > 0 && (
            ...
            >
              ...
              {inputs.availableOptions.map((item, index) => {
                return (
                  <div
                    ...
                    key={index}
                  >
                    {Object.hasOwn(inputs, 'optionHasId') && !inputs.optionHasId && (
                      <EditIcon
                        onClick={() => {
                          const cloned = structuredClone(item);
                          cloned.coast = cloned.coast == null ? '' : String(cloned.coast);
                          cloned.previousPrice =
                            cloned.previousPrice == null ? '' : String(cloned.previousPrice);
                          cloned.qouantity = cloned.qouantity == null ? '' : String(cloned.qouantity);
                          cloned.price = cloned.price == null ? '' : String(cloned.price);
                          setEditingOptionValues(cloned);
                        }}
                        sx={{ position: 'absolute', top: 0, left: 0, color: '#03a9f4', cursor: 'pointer' }}
                      />
                    )}
                    <CloseOutlinedIcon
                    ...
                    <Typography
                      ...
                      onClick={() => {
                        if (
                          (Object.hasOwn(inputs, 'optionHasId') && inputs.optionHasId) ||
                          !Object.hasOwn(inputs, 'optionHasId')
                        ) {
                          setProduct(item.id);
                        }
                      }}
              ...
              {editingOptionValues && (
                ...
                >
                  <Typography sx={{ fontWeight: 500, mb: '8px' }}>Edit Option</Typography>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <TextField
                      size="small"
                      label="Coast"
                      type="number"
                      value={editingOptionValues.coast ?? ''}
                      onChange={(e) => setEditingOptionValues((prev) => ({ ...prev, coast: e.target.value }))}
                    />
                    <TextField
                      size="small"
                      label="Previous Price"
                      type="number"
                      value={editingOptionValues.previousPrice ?? ''}
                      onChange={(e) =>
                        setEditingOptionValues((prev) => ({ ...prev, previousPrice: e.target.value }))
                      }
                    />
                    <TextField
                      size="small"
                      label="Size"
                      value={editingOptionValues.size ?? ''}
                      onChange={(e) => setEditingOptionValues((prev) => ({ ...prev, size: e.target.value }))}
                    />
                    <TextField
                      size="small"
                      label="Quantity"
                      type="number"
                      value={editingOptionValues.qouantity ?? ''}
                      onChange={(e) =>
                        setEditingOptionValues((prev) => ({ ...prev, qouantity: e.target.value }))
                      }
                    />
                    <TextField
                      size="small"
                      label="Number"
                      value={editingOptionValues.number ?? ''}
                      onChange={(e) =>
                        setEditingOptionValues((prev) => ({ ...prev, number: e.target.value }))
                      }
                    />
                    <TextField
                      size="small"
                      label="Price"
                      type="number"
                      value={editingOptionValues.price ?? ''}
                      onChange={(e) => setEditingOptionValues((prev) => ({ ...prev, price: e.target.value }))}
                    />
                    <FormControlLabel
                      control={
                        <Switch
                          size="small"
                          checked={Boolean(editingOptionValues.inStock)}
                          onChange={(e) =>
                            setEditingOptionValues((prev) => ({ ...prev, inStock: e.target.checked }))
                          }
                        />
                      }
                      label="In Stock"
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const cleaned = {
                          ...editingOptionValues,
                          coast:
                            editingOptionValues.coast === '' || editingOptionValues.coast == null
                              ? ''
                              : Number(editingOptionValues.coast),
                          previousPrice:
                            editingOptionValues.previousPrice === '' ||
                            editingOptionValues.previousPrice == null
                              ? ''
                              : Number(editingOptionValues.previousPrice),
                          qouantity:
                            editingOptionValues.qouantity === '' || editingOptionValues.qouantity == null
                              ? ''
                              : Number(editingOptionValues.qouantity),
                          price:
                            editingOptionValues.price === '' || editingOptionValues.price == null
                              ? ''
                              : Number(editingOptionValues.price),
                        };
                        setInputs((prev) => ({
                          ...prev,
                          availableOptions: prev.availableOptions.map((opt) =>
                            opt.id === cleaned.id ? { ...cleaned } : opt
                          ),
                        }));
                        setEditingOptionValues(null);
                        setEditInitialProductOption(null);
                      }}
                    >
                      Save
                    </Button>
        ...
        <ExtraProductOptions
          ...
          disableSwitch={editedProduct?.availableOptions?.length > 0 ? true : false}
        />
        <ImageInputs
          requiredFields={requiredFields}
          inputs={inputs}
